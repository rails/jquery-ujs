...
Rails.serializeElement = (element, additionalParam) ->
  ...
  inputs.forEach (input) ->
    ...
    else if input.type != 'radio' and input.type != 'checkbox' or input.checked
...
Rails.blankInputs = (form, selector, nonBlank) ->
  ...
  requiredInputs.forEach (input) ->
    if input.type == 'radio'
      ...
      valueToCheck = if input.type == 'checkbox' then input.checked else !!input.value
      foundInputs.push input if valueToCheck == nonBlank
  foundInputs
