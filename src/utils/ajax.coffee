...
Rails.ajax = (options) ->
  ...
  if options.type == 'GET' and options.data
  ...
  accept += ', */*; q=0.01' if options.dataType != '*'
  ...
  httpRequest.setRequestHeader 'Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8' if typeof options.data == 'string'
  httpRequest.setRequestHeader 'X-Requested-With', 'XMLHttpRequest' unless options.crossDomain
  # Add X-CSRF-Token
  CSRFProtection httpRequest
  httpRequest.withCredentials = true if options.withCredentials
  httpRequest.onreadystatechange = ->
    if httpRequest.readyState == XMLHttpRequest.DONE
      ajaxHandleResponses httpRequest, options.success, options.error, options.complete

  # Call beforeSend hook
  if options.beforeSend? and options.beforeSend(httpRequest, options) == false
    httpRequest.abort()
...
ajaxHandleResponses = (httpRequest, success, error, complete) ->
  ...
  if typeof response == 'string' and typeof type == 'string'
